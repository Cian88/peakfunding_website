...
export default function SimulateurCapacite({ lang = 'fr' }: { lang?: Lang }) {
  ...
  return (
    <form className="grid gap-5" onSubmit={(e) => e.preventDefault()} aria-labelledby="sim-titre">
      ...
      {projet === 'loc' && (
        <label className="grid gap-2"><span className="libelle">{S.loyers} <span className="font-normal text-argent">{S.loyers_note}</span></span>
          <span className="relative"><input className="champ pr-20" type="number" min={0} step={50} value={loyers} onChange={(e) => setLoyers(+e.target.value)} /><span className="pointer-events-none absolute top-1/2 right-4 -translate-y-1/2 font-mono text-[12px] text-argent">{S.mois}</span></span>
        </label>
      )}
